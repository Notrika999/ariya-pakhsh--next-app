        ...
        <div className="bg-white xl:sticky fixed xl:top-0 bottom-0 inset-e-0 inset-s-0 xl:z-0 z-40 dark:bg-custom-dark dark:text-gray-200 space-y-3 shadow-sm border border-gray-200 dark:border-gray-700 rounded-2xl px-6 py-4">
          ...
          <div className="bg-gray-100/90 xl:block hidden dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-2xl shadow py-5 px-3 space-y-5">
            ...
            <div className="rounded-xl py-4 px-2 space-y-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm font-bold text-gray-800 dark:text-white">عملکرد</span>
                <span className="text-xs text-green-600 dark:text-green-400">عالی</span>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm font-bold text-gray-800 dark:text-white">وضعیت موجودی</span>
                {isOutOfStock ? (
                  ...
                  <span className="text-xs text-green-600">
                    موجود ({new Intl.NumberFormat("fa-IR").format(variant?.availableQuantity ?? 0)} عدد)
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm font-bold text-gray-800 dark:text-white">ارسال از فروشگاه اصلی</span>
                <span className="text-xs text-gray-500 dark:text-gray-300">آماده ارسال</span>
              </div>

              {/* Tags */}
              <div className="flex items-center gap-2 pt-2 flex-wrap">
                <span className="text-xs bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 px-3 py-1 rounded-full">کالای اصل</span>
                <span className="text-xs bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 px-3 py-1 rounded-full">کالای نو</span>
                ...
                {isOutOfStock && (
                  <span className="text-xs bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 px-3 py-1 rounded-full">ناموجود</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="overflow-hidden h-10 my-3">
            <div className="h-10 flex text-sm items-center text-gray-700 dark:text-gray-300">
              👁️ <b className="mx-1">{new Intl.NumberFormat("fa-IR").format(product?.viewCount ?? 0)}</b> نفر این کالا را مشاهده کرده‌اند
            </div>
          </div>
                ...
                {hasDiscount && (
                  <div className="flex justify-between items-center">
                    <del className="text-zinc-400 dark:text-zinc-500">
                      <span>{new Intl.NumberFormat("fa-IR").format(originalPrice)}</span>
                    </del>
                    <div className={`text-white text-xs ms-2 font-bold px-2 py-1 rounded-xl rounded-bl-md shadow shadow-red-500/50 z-10 ${isOutOfStock ? "bg-gray-400" : "bg-secondary-500"}`}>
                      {discountPercent}%
                    </div>
                  </div>
                )}
                <span className={`text-xl inline-block mt-2 font-bold ${isOutOfStock ? "text-gray-300" : "dark:text-white"}`}>
                  {new Intl.NumberFormat("fa-IR").format(price)}
                </span>
              </div>
              <span className="text-xs font-bold -rotate-90 dark:text-zinc-300">تومان</span>
            </div>
          </div>
