...
- `withAuth: true`:
  - خواندن کوکی‌های `CUP_Access_Token`، `CUP_Refresh_Token`، `CUP_Device_Id`
...
| نام کوکی | کاربرد |
|----------|--------|
| `CUP_Access_Token` | Access JWT |
| `CUP_Refresh_Token` | Refresh |
| `CUP_Device_Id` | Device / fingerprint سمت سرور |
